...
static void _print_strings(RCore *r, RList *list, int mode, int va) {
	...
	r_list_foreach (list, iter, string) {
		...
		if (IS_MODE_SET (mode)) {
			...
			r_meta_add (r->anal, R_META_TYPE_STRING, vaddr, string->size, string->string);
...
static void set_bin_relocs(RCore *r, RBinReloc *reloc, ut64 addr, Sdb **db, char **sdb_module) {
	...
	if (is_pe && !is_sandbox && reloc->import
		...
		r_meta_add (r->anal, R_META_TYPE_DATA, reloc->vaddr, 4, NULL);
	}
...
static void add_metadata(RCore *r, RBinReloc *reloc, ut64 addr, int mode) {
	...
	if (IS_MODE_SET (mode)) {
		r_meta_add (r->anal, R_META_TYPE_DATA, reloc->vaddr, cdsz, NULL);
	} else if (IS_MODE_RAD (mode)) {
	...
	r_list_foreach (imports, iter, import) {
		...
		if (IS_MODE_SET (mode)) {
			...
			if (libname && strstr (libname, ".dll") && cdsz) {
				r_meta_add (r->anal, R_META_TYPE_DATA, addr, cdsz, NULL);
			}
		} else if (IS_MODE_SIMPLE (mode)) {
			r_cons_printf ("%s%s%s\n",
	...
	r_list_foreach (sections, iter, section) {
		...
		if (IS_MODE_RAD (mode)) {
			...
			if (!section->is_segment || segments_only) {
				...
				r_meta_add (r->anal, R_META_TYPE_COMMENT, addr, 1, str);
				R_FREE (str);
			}
			if (section->add) {
				bool found;
